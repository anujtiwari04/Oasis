import { useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { ChevronRight, Menu, X, Phone } from "lucide-react";
import oasisLogo from "@/assets/Oasis Logo.png";
import postalBallotNotice from "@/assets/postal-ballot-notice.pdf";
import compositionOfCommittees from "@/assets/COMPOSITION OF COMMITTEES.pdf";
import bookClosure2025 from "@/assets/book-closure-2025.pdf";
import outcomeOfBoardMeeting from "@/assets/OUTCOME OF BOARD MEETING.pdf";
import dlofAsset from "@/assets/DLOF.pdf";

// Open Offers 2023 PDF Imports
import openOfferPA from "@/assets/Open offers/PA-Oasis_Securities_Limited.pdf";
import openOfferDPS from "@/assets/Open offers/DPS-Oasis.pdf";
import openOfferDLOF from "@/assets/Open offers/dlof_osl.pdf";
import openOfferCorrDPS from "@/assets/Open offers/Corrigendum to DPS - Oasis_30.05.2024.pdf";
import openOfferCorrLOF from "@/assets/Open offers/Corrigendum to LOF - Oasis_07.06.2024.pdf";
import openOfferLOF from "@/assets/Open offers/LOF - Oasis Securities Limited 30.05.2024.pdf";

const postalBallotItems = [
  { href: postalBallotNotice, label: "Notice", download: "postal-ballot-notice.pdf" },
  { href: "/scrutinizers-report", label: "Scrutinizer's Report" },
  { href: "#", label: "Voting Result and Scrutinizer Report" },
];

const corporateItems = [
  { href: "#", label: "Postal Ballot", children: postalBallotItems },
  { href: compositionOfCommittees, label: "Composition of Committees", download: "COMPOSITION OF COMMITTEES.pdf" },
  { href: "/board-of-directors", label: "Board of Directors" },
  { href: "/key-management-personnel", label: "Key Management Personnel" },
  { href: "/compliance-officer", label: "Compliance Officer" },
  { href: "/policies", label: "Policies" },
  { href: "/agm-notices", label: "AGM/EOGM Notices" },
  { href: "/board-meetings-notices", label: "Board Meetings Notices" },
  { href: "/financial-results", label: "Financial Results" },
  { href: "/shareholding-pattern", label: "Shareholding pattern" },
  { href: "/share-transfer-agent", label: "Share Transfer Agent (RTA)" },
  { href: "/rsca-reports", label: "RSCA Reports" },
  { href: "/investor-grievance", label: "Investor Grievance" },
  { href: "/annual-reports", label: "Annual Reports" },
  { href: bookClosure2025, label: "Book Closure", download: "book-closure-2025.pdf" },
  { href: "/agm-results", label: "AGM/EOGM Results" },
  { href: "#", label: "Circulars & Disclosures" },
  { href: "/forms", label: "Forms" },
  { href: "/newspaper-publications", label: "Newspaper Publications" },
  { href: outcomeOfBoardMeeting, label: "Outcome", download: "OUTCOME OF BOARD MEETING.pdf" },
  { href: dlofAsset, label: "DLOF", download: "DLOF.pdf" },
];

const openOffers2023Items = [
  { href: openOfferPA, label: "Public Announcement", download: "PA-Oasis_Securities_Limited.pdf" },
  { href: openOfferDPS, label: "Detailed Public Statement", download: "DPS-Oasis.pdf" },
  { href: openOfferDLOF, label: "Draft Letter of Offer", download: "dlof_osl.pdf" },
  { href: openOfferCorrDPS, label: "Corrigendum to DPS", download: "Corrigendum to DPS - Oasis_30.05.2024.pdf" },
  { href: openOfferCorrLOF, label: "Corrigendum to LOF", download: "Corrigendum to LOF - Oasis_07.06.2024.pdf" },
  { href: openOfferLOF, label: "LOF-OasisSecurities Limited", download: "LOF - Oasis Securities Limited 30.05.2024.pdf" },
];

interface NavItem {
  href: string;
  label: string;
  download?: string;
  children?: NavItem[];
}

const navLinks: NavItem[] = [
  { href: "/", label: "Home" },
  { href: "/#about", label: "About" },
  { href: "/#corporate", label: "Corporate", children: corporateItems },
  // { href: "/#open-offers", label: "Open Offers 2023", children: openOffers2023Items },
  // { href: "/#links", label: "Links" },
  { href: "/contact", label: "Contact" },
];

export function Header() {
  const [open, setOpen] = useState(false);
  const [activeCorporateSubmenu, setActiveCorporateSubmenu] = useState<string | null>(null);

  return (
    <>
      {/* Top utility bar */}
      <div className="bg-brand-green text-white text-sm py-2">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex flex-wrap items-center justify-between gap-2">
          <a href="tel:9257056969" className="flex items-center gap-2 hover:text-brand-gold transition-colors">
            <Phone className="h-3.5 w-3.5" />
            <span>Call Us: 9257056969</span>
          </a>
          <span className="text-white/60 text-xs sm:text-sm">CIN: L51900MH1986PLC041499</span>
        </div>
      </div>

      {/* Sticky header */}
      <header className="bg-white/90 backdrop-blur-md sticky top-0 z-50 border-b border-brand-green/10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid h-16 grid-cols-[1fr_auto_1fr] items-center">
            <a href="/" className="flex items-center gap-2" aria-label="Oasis Securities Limited home">
              <img src={oasisLogo} alt="Oasis Securities Limited" className="h-9 w-auto object-contain" />
              <span className="font-bold tracking-tight text-brand-charcoal text-sm sm:text-base">
                OASIS SECURITIES LTD
              </span>
            </a>

            <nav className="hidden md:flex items-center justify-center gap-8" aria-label="Primary">
              {navLinks.map((l) =>
                l.children ? (
                  <div key={l.href} className="group relative py-5">
                    <a
                      href={l.href}
                      className="text-sm font-medium text-brand-text/60 hover:text-brand-charcoal transition-colors relative after:content-[''] after:absolute after:left-0 after:-bottom-1 after:h-0.5 after:w-0 after:bg-brand-green group-hover:after:w-full after:transition-all"
                    >
                      {l.label}
                    </a>

                    <div
                      className="invisible absolute left-0 top-full opacity-0 transition-all duration-150 group-hover:visible group-hover:opacity-100"
                      onMouseLeave={() => setActiveCorporateSubmenu(null)}
                    >
                      <div className="relative w-72">
                        <ul
                          className="max-h-[500px] overflow-y-auto overflow-x-hidden bg-white text-brand-text shadow-lg ring-1 ring-black/5 border-t-2 border-brand-green"
                          aria-label={`${l.label} menu`}
                        >
                          {l.children.map((item) => (
                            <li
                              key={item.label}
                              className="border-b border-brand-charcoal/5 last:border-b-0"
                              onMouseEnter={() => setActiveCorporateSubmenu(item.children ? item.label : null)}
                            >
                              <a
                                href={item.href}
                                download={item.download}
                                className="flex min-h-11 items-center justify-between gap-3 px-4 py-3 text-sm font-semibold leading-snug hover:bg-brand-green/5 hover:text-brand-green focus:bg-brand-green/5 focus:text-brand-green focus:outline-none transition-colors"
                              >
                                <span>{item.label}</span>

                                {item.children ? (
                                  <ChevronRight className="h-4 w-4 shrink-0" aria-hidden="true" />
                                ) : null}
                              </a>
                            </li>
                          ))}
                        </ul>

                        {l.label === "Corporate" && activeCorporateSubmenu === "Postal Ballot" ? (
                          <ul className="absolute left-full top-0 z-10 w-72 bg-white text-brand-text shadow-lg ring-1 ring-black/5 border-t-2 border-brand-green">
                            {postalBallotItems.map((child) => (
                              <li key={child.label} className="border-b border-brand-charcoal/5 last:border-b-0">
                                <a
                                  href={child.href}
                                  download={child.download}
                                  className="block min-h-11 px-4 py-3 text-sm font-semibold leading-snug hover:bg-brand-green/5 hover:text-brand-green focus:bg-brand-green/5 focus:text-brand-green focus:outline-none transition-colors"
                                >
                                  {child.label}
                                </a>
                              </li>
                            ))}
                          </ul>
                        ) : null}
                      </div>
                    </div>
                  </div>
                ) : (
                  <a
                    key={l.href}
                    href={l.href}
                    className="text-sm font-medium text-brand-text/60 hover:text-brand-charcoal transition-colors relative after:content-[''] after:absolute after:left-0 after:-bottom-1 after:h-0.5 after:w-0 after:bg-brand-green hover:after:w-full after:transition-all"
                  >
                    {l.label}
                  </a>
                )
              )}
            </nav>

            <div className="hidden md:flex justify-end">
              <a
                href="/contact"
                className="inline-flex items-center rounded-[4px] bg-brand-green px-4 py-2 text-sm font-medium text-white hover:bg-brand-green-light transition-colors"
              >
                Get in Touch
              </a>
            </div>

            <button
              className="md:hidden col-start-3 justify-self-end inline-flex items-center justify-center p-2 text-brand-charcoal"
              onClick={() => setOpen(true)}
              aria-label="Open menu"
            >
              <Menu className="h-6 w-6" />
            </button>
          </div>
        </div>
      </header>

      {/* Mobile drawer */}
      <AnimatePresence>
        {open && (
          <>
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => setOpen(false)}
              className="fixed inset-0 z-[60] bg-brand-charcoal/50 backdrop-blur-sm md:hidden"
            />
            <motion.aside
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
              transition={{ type: "tween", duration: 0.3 }}
              className="fixed top-0 right-0 bottom-0 z-[70] w-72 bg-white shadow-xl md:hidden flex flex-col"
              aria-label="Mobile menu"
            >
              <div className="flex items-center justify-between p-4 border-b border-brand-green/10">
                <span className="font-bold text-brand-charcoal">OASIS SECURITIES</span>
                <button onClick={() => setOpen(false)} aria-label="Close menu" className="p-2">
                  <X className="h-5 w-5 text-brand-charcoal" />
                </button>
              </div>

              <nav className="flex flex-col p-4 gap-1 overflow-y-auto" aria-label="Mobile primary">
                {navLinks.map((l) =>
                  l.children ? (
                    <div key={l.href} className="rounded-[4px] border border-brand-green/10 mb-1">
                      <a
                        href={l.href}
                        onClick={() => setOpen(false)}
                        className="block px-3 py-3 text-brand-charcoal font-semibold transition-colors hover:bg-brand-green/5"
                      >
                        {l.label}
                      </a>
                      <div className="border-t border-brand-green/10 bg-brand-surface-alt py-1">
                        {l.children.map((item) => (
                          <div key={item.label}>
                            <a
                              href={item.href}
                              download={item.download}
                              onClick={() => setOpen(false)}
                              className="block px-5 py-2.5 text-sm font-medium text-brand-text/70 hover:text-brand-green transition-colors"
                            >
                              {item.label}
                            </a>
                            {item.children ? (
                              <div className="pb-1">
                                {item.children.map((child) => (
                                  <a
                                    key={child.label}
                                    href={child.href}
                                    download={child.download}
                                    onClick={() => setOpen(false)}
                                    className="block px-8 py-2 text-sm text-brand-text/50 hover:text-brand-green transition-colors"
                                  >
                                    {child.label}
                                  </a>
                                ))}
                              </div>
                            ) : null}
                          </div>
                        ))}
                      </div>
                    </div>
                  ) : (
                    <a
                      key={l.href}
                      href={l.href}
                      onClick={() => setOpen(false)}
                      className="px-3 py-3 rounded-[4px] text-brand-text/70 hover:bg-brand-green/5 hover:text-brand-green font-medium transition-colors mb-1"
                    >
                      {l.label}
                    </a>
                  )
                )}
              </nav>
            </motion.aside>
          </>
        )}
      </AnimatePresence>
    </>
  );
}
